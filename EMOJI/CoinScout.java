...
public class CoinScout implements RobotBehavior {

    int currentTurns;
    PathOptionGenerator pathOptionGenerator;
    Robot robot;

    public CoinScout(int currentTurns, Robot robot, PathOptionGenerator pathOptionGenerator) {
    ...
    public Command getCommand(Robot robot, Location location) {
        ...
        if (location.getCoins().size() > 0) command = new CommandCoin(robot);
        else command = new CommandMove(robot, dirction);
        currentTurns -= 1;
        return command;        
